...
void c_scan(int initial_head, int requests[], int total_requests, int max_tracks, int direction) {
    ...
    if (direction == 0) { // Towards lower numbered tracks
        ...
        // Jump to the highest numbered track
        total_movement += max_tracks - 1 - current_head;
        current_head = max_tracks - 1;
        printf("%d -> ", current_head);

        // Service requests after the initial head position until reaching the initial head position
        for (i = max_tracks - 1; i > initial_head; i--) {
            if (requests[i] != -1) {
                total_movement += abs(current_head - requests[i]);
                current_head = requests[i];
                requests[i] = -1; // Mark request as serviced
        ...
        // Jump to the lowest numbered track
        total_movement += current_head;
        current_head = 0;
        printf("%d -> ", current_head);

        // Service requests before the initial head position until reaching the initial head position
        for (i = 0; i < initial_head; i++) {
            if (requests[i] != -1) {
                total_movement += abs(current_head - requests[i]);
                current_head = requests[i];
                requests[i] = -1; // Mark request as serviced
...
int main() {
    ...
    c_scan(initial_head, requests, total_requests, max_tracks, direction);
